...
class Solution:
    def splitArray(self, nums: List[int], k: int) -> int:
        ...
        def dfs(i, m):
            ...
            for j in range(i , n - m + 1): #thi sis tricky, this loop tries to tell us all possible indices j that the group can end, 
                # why n - m + 1? we have to make sure that there is at one element for each of the remaining (m - 1) groups
                curSum += nums[j]
