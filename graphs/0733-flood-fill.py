...
# You are also given three integers sr, sc, and color. Your task is to perform a flood fill on the image starting 
...
class Solution:
    def floodFill(self, image: List[List[int]], sr: int, sc: int, color: int) -> List[List[int]]:
        #each square represents the pixel value of image
        #sr
        #sc
        #color
        ...
        #1. change start with color
        #2. adjacent also changes, horizontally or vertically (x, y ) and shares same color as starting pixel

        #3. keep repeating
        #4. stops when no more


        startingColor = image[sr][sc]

        if startingColor == color:
            return image #if starting color already matches no change needed
        ...
        while q :
            ...
            for dr, dc in directions:
                nr, nc = r + dr, c + dc
                #if inbound add to process
                if 0 <= nr < rows and 0 <= nc < cols and image[nr][nc] == startingColor:
                    image[nr][nc] = color
                    q.append((nr,nc))
