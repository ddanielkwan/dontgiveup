...
class Solution:
    def findAllPeople(self, n: int, meetings: List[List[int]], firstPerson: int) -> List[int]:
        ...
        for t in sorted(time_map.keys()): #all unique times
            visit = set()
            for src_node in time_map[t]: #in this adj list, we have every edge in that adj list,
                #  we want to consieder every single key in this adj list and consdier as src node,
                #  we want to contemplate shold we run a dfs or not from this src, we onyl run if the 
                # src is in secrets because only then can secret be spread to other nodes
