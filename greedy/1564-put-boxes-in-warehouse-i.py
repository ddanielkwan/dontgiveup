...
class Solution:
    def maxBoxesInWarehouse(self, boxes: List[int], warehouse: List[int]) -> int:
        ...
        for i in range(1, len(warehouse)): #step 1
            warehouse[i] = min(warehouse[i], warehouse[i-1])

        boxes.sort()
        warehouse.sort() #you might think we are rearranging the warehouse but we are not
        #all rooms are valid, since we already did the contraint  in step1

        #now the new questino becomes, match what box to what room
