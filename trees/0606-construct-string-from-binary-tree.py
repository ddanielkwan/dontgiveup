...
# 
# 
# ->>>>>>>>>>>> However, when a node has a right child but no left child, 
# an empty pair of parentheses must precede the representation of the
...
class Solution:
    def tree2str(self, root: Optional[TreeNode]) -> str:
        ...
        def dfs(node):
            ...
            res += str(node.val) #preorder
            if node.left or (not node.left and node.right): #from problem desc
                res += "("
                dfs(node.left)
                res += ")"
            if node.right:
