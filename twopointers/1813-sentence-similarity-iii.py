...
class Solution:
    ...
    def areSentencesSimilar(self, sentence1: str, sentence2: str) -> bool:
        ...
        #only modify smaller string,
        #makes sense to only modify smaller string because cant delete
        if len(s2) < len(s1):
            s1, s2 = s2, s1

        #three cases true if
        #1. smaller one is prefix of larger
        #2. smaller one is suffix of larger
        #3. we need something in between
        ...
        #if pointers cross then we true
        #because smallerone has to get matched for all words
        #e.g my name vs my big name
        #
        return l1 > r1
